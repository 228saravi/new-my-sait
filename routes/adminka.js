...
router.post('/addJob', function (req, res) {
  ...
  if (!req.body.head || !req.body['condision[]'] || !req.body['requir[]']) {
    ...
    return res.json({status: false,message: 'Укажите данные!'});
  }
  ...
  item.save().then(
    //обрабатываем и отправляем ответ в браузер
    (i) => {
      return res.json({status: true, message: 'Запись успешно добавлена'});
    }, e => {
    ...
    res.json({
      status: false, message: 'При добавление записи произошла ошибка: ' + error
    });
  });
});
